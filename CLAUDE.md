...
- **Teach concepts before code.** For each new sub-section, explain what we're building and why *before* writing anything, so the user holds the idea in mind while coding. **Keep the concept explanation as simple and concise as possible — direct to the point, no padding.** The user grasps things fast; say the minimum that makes the idea land.
- **Scaffold split.** Guide the user to write only the genuinely-new, concept-bearing lines. For anything already written in a prior exercise (imports, `ClaudeAgentOptions` block, stream/drain loop, usage-tracker wiring, `main()` skeleton), hand it over as ready-made scaffold with a clearly marked `# YOU: ...` hole for the new part — don't make the user retype boilerplate. **Once the user greenlights the exercise, write the scaffold directly — don't pre-announce what it will contain or wait for a second okay.**
- **Actually write the scaffold file to disk** with the Write tool — create the file, drop in all the boilerplate, and leave the `# YOU (hole N): ...` markers in place. Then in chat hand back *only the snippets that fill each hole*. Never dump the whole file into the message for the user to type out wholesale — they fill the holes, not the boilerplate. (Still show the exact fill-in code per hole; never leave a hole blank.)
...
- **Never mention a feature, field, or behavior without showing it.** If you name something (`__required_keys__`, a flag, a method, an attribute), show the concrete usage — a code snippet or exact call — in the same breath. No dangling "X does Y" facts left for the user to go figure out how to use. If it's not worth a usage example, don't bring it up.
